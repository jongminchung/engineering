...
```bash
# Kafka 클러스터 시작
docker-compose -f compomse.yml up -d

# 로그 확인
docker-compose -f compomse.yml logs -f

# 중지
docker-compose -f compomse.yml down

# 볼륨까지 삭제 (데이터 초기화)
docker-compose -f compomse.yml down -v
```
...
```bash
docker-compose -f compomse.yml restart
```
