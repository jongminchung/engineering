...
- This module provides a distributed lock library with Spring integration and Spring Boot starters.
- Submodules:
  ...
  - `distributed-lock/spring-boot-autoconfigure` — auto-configuration for providers.
  - `distributed-lock/spring-boot-starter` and `distributed-lock/spring-boot-starter-*` — starters.
...
- `./gradlew :distributed-lock:spring-boot-autoconfigure:test` — run auto-config tests.
- `./gradlew :distributed-lock:test:test` — run test-utility module tests.
- `./gradlew :distributed-lock:dependencies:dependencies` — inspect module dependency graph.
